...
/**
 * The master-data twin of `BrandForm` — same fields, same visual chrome
 * (`odoo-form-wrapper` / `record-grid`, the same one the Pelanggan/Client
 * form uses), but no outreach affordances like the Meeting/Call quick-task
 * drawer: this page is about the record itself, not the funnel around it.
 * Saves through its own actions so it lands back on `/brand-management`,
 * not Tracker's `/brand`.
 */
export function BrandManagementForm({ brand, members }: { brand: Brand | null; members: Member[] }) {
  const action = brand ? updateBrandFromManagement : createBrandFromManagement;
  ...
  return (
    <>
      <form id="brand-management-form" action={formAction} className="odoo-form-wrapper" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        <CsrfField />
        ...
        <div className="odoo-control-panel">
          <div className="odoo-cp-top">
            <div className="odoo-cp-breadcrumb">
              <Link href="/brand-management" style={{ color: 'var(--ink-2)', marginRight: 8, textDecoration: 'none' }}>
          ...
          <div className="odoo-cp-bottom">
            <div className="odoo-cp-actions">
              ...
              <Link href="/brand-management" className="btn ghost">{t.brand.discard}</Link>
              {brand ? (
          ...
          <div className="record-sheet" style={{ margin: '0 auto', width: '100%', maxWidth: 1240, marginTop: 16 }}>
            <div className="record-grid">
              <div className="record-field" style={{ gridColumn: '1 / -1' }}>
                <label htmlFor="mgmt-name">{t.brand.name}</label>
                <input className="line-input" id="mgmt-name" name="name" required
                       defaultValue={brand?.name ?? ''} placeholder={t.brand.namePlaceholder} />
              </div>

              <div className="record-field">
                <label htmlFor="mgmt-picName">{t.brand.picName}</label>
                <input className="line-input" id="mgmt-picName" name="picName"
                       defaultValue={brand?.picName ?? ''} placeholder={t.brand.picNamePlaceholder} />
              </div>

              <div className="record-field">
                <label htmlFor="mgmt-phone">{t.brand.phone}</label>
                <input className="line-input" id="mgmt-phone" name="phone" defaultValue={brand?.phone ?? ''}
                       placeholder="+62812xxxxxxx" disabled={phoneMasked} />
                {phoneMasked ? <p className="record-hint">{t.client.phoneMaskedHint}</p> : null}
              </div>

              <div className="record-field">
                <label htmlFor="mgmt-email">{t.brand.email}</label>
                <input className="line-input" id="mgmt-email" name="email" type="email"
                       defaultValue={brand?.email ?? ''} placeholder="nama@brand.com" />
              </div>

              <div className="record-field">
                <label htmlFor="mgmt-instagram">{t.brand.instagram}</label>
                <input className="line-input" id="mgmt-instagram" name="instagram"
                       defaultValue={brand?.instagram ?? ''} placeholder={t.brand.instagramPlaceholder} />
              </div>

              <div className="record-field">
                <label htmlFor="mgmt-website">{t.brand.website}</label>
                <input className="line-input" id="mgmt-website" name="website"
                       defaultValue={brand?.website ?? ''} placeholder="https://…" />
              </div>

              <div className="record-field">
                <label htmlFor="mgmt-category">{t.brand.category}</label>
                <input className="line-input" id="mgmt-category" name="category"
                       defaultValue={brand?.category ?? ''} placeholder={t.brand.categoryPlaceholder} />
              </div>

              <div className="record-field">
                <label htmlFor="mgmt-city">{t.brand.city}</label>
                <input className="line-input" id="mgmt-city" name="city" defaultValue={brand?.city ?? ''} />
              </div>

              <div className="record-field">
                <label htmlFor="mgmt-source">{t.brand.source}</label>
                <select className="line-input" id="mgmt-source" name="source" defaultValue={brand?.source ?? 'manual'}>
              ...
              <div className="record-field">
                <label htmlFor="mgmt-assigneeId">{t.brand.assignee}</label>
                <select className="line-input" id="mgmt-assigneeId" name="assigneeId" defaultValue={brand?.assigneeId ?? ''}>
              ...
              {brand ? (
                <div className="record-field">
                  <label>{t.brand.status}</label>
                  <p className="record-static">{t.brand.statusLabel[brand.status]}</p>
                  <p className="record-hint">{t.brandManagement.statusHint}</p>
                </div>
              ) : null}

              <div className="record-field" style={{ gridColumn: '1 / -1' }}>
                <label htmlFor="mgmt-notes">{t.brand.notes}</label>
                <textarea className="line-input" id="mgmt-notes" name="notes" rows={3}
