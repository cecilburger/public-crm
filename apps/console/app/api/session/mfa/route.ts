...
export async function POST(req: NextRequest) {
  ...
  const back = (message: string) => {
    const url = new URL(withBase('/masuk/kode'), req.url);
  ...
  if (!mfaToken || !workspace) {
    return NextResponse.redirect(new URL(withBase('/masuk?reason=expired'), req.url), { status: 303 });
  }
  ...
  try {
    ...
    const res = NextResponse.redirect(new URL(withBase('/obrolan'), req.url), { status: 303 });
