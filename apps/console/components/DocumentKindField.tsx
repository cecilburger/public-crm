...
import { addDocumentKind } from '@/app/(app)/actions';
import { useCsrfToken } from '@/components/Csrf';
import { t } from '@/lib/copy';
import type { DocumentKind } from '@/lib/api';

const BUILTIN = ['penawaran', 'invoice', 'kwitansi', 'lainnya'] as const;

/**
 * The Jenis select, plus an inline "+ Tambah Jenis Baru" — same shape as
 * `TaskKindField`, one dropdown down from Tugas.
 */
export function DocumentKindField({
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      fd.set('csrf', csrf);
      fd.set('name', trimmed);
      const res = await addDocumentKind(fd);
      if (!res.ok || !res.kind) {
        setError(res.error ?? t.document.addKindFailed);
        return;
      }
  ...
  return (
    <div className="record-field">
      <label htmlFor={id}>{t.document.kind}</label>
      <select className="line-input" id={id} value={adding ? '__add__' : value}
              onChange={(e) => handleSelect(e.target.value)}>
        {BUILTIN.map((k) => <option key={k} value={k}>{t.document.kindLabel[k]}</option>)}
        {customKinds.map((k) => <option key={k.id} value={k.name}>{k.name}</option>)}
        <option value="__add__">+ {t.document.addKind}</option>
      </select>
      ...
      {adding ? (
        <span style={{ display: 'flex', gap: 6, marginTop: 6, alignItems: 'center' }}>
          <input className="line-input" value={newName} onChange={(e) => setNewName(e.target.value)}
                 placeholder={t.document.addKindPlaceholder} autoFocus
                 onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAdd(); } }} />
          <button type="button" className="btn ghost sm" onClick={handleAdd} disabled={pending || !newName.trim()}
                  aria-label={t.document.addKind} title={t.document.addKind}>
            +
          </button>
