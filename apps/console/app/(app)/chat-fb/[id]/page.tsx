...
export default async function ChatFbThreadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Facebook Page was disconnected, cascading its conversations with it.
    // The list view is still there, so send the agent back to it rather than
    // a dead end.
    if (err instanceof ApiError && err.status === 404) redirect('/chat-fb');
    throw err;
  }
    ...
    <div style={{ display: 'flex', minHeight: 0 }}>
      <div className="thread" style={{ flex: 1, minWidth: 0 }}>
        ...
        {/* Messenger has no 24-hour standard-messaging window enforced here —
            same reasoning as Chat IG's composer: nothing server-side to apply
            it against yet, so it stays open rather than showing a restriction
            that isn't real. */}
