...
async function applyActions(
  ...
  const next = {
    ...
    // `node`/`outcome` are only taken when the booking call actually set
    // them: a failed or slot-unavailable booking can come back with either
    // empty, and overwriting `next.node` with an empty string would erase
    // the value `set_node` above already picked.
    ...(booking ? {
