...
export type TemplateChannel = 'whatsapp' | 'email' | 'other';
export type TemplateCategory = 'marketing' | 'utility' | 'authentication';
export type TemplateStatus = 'draft' | 'pending' | 'approved' | 'rejected';

export interface MessageTemplateRow {
  id: string; name: string; channel: TemplateChannel; category: TemplateCategory; language: string; body: string;
...
function mapRow(r: TemplateDbRow): MessageTemplateRow {
  return {
    id: r.id, name: r.name, channel: r.channel, category: r.category, language: r.language, body: r.body,
    status: r.status, notes: r.notes,
    createdBy: r.created_by, createdAt: r.created_at, updatedAt: r.updated_at,
  };
}

const COLUMNS = 'id, name, channel, category, language, body, status, notes, created_by, created_at, updated_at';
...
export async function createMessageTemplate(
  ctx: Ctx,
  args: {
    name: string; channel?: TemplateChannel; category?: TemplateCategory; language?: string; body: string;
    status?: TemplateStatus; notes?: string | null; createdBy: string;
  },
): Promise<{ id: string }> {
  const rows = await ctx.tx.query<{ id: string }>(
    `insert into message_templates (tenant_id, name, channel, category, language, body, status, notes, created_by)
     values ($1,$2,$3,$4,$5,$6,$7,$8,$9)
     returning id`,
    [ctx.tenantId, args.name, args.channel ?? 'whatsapp', args.category ?? 'utility', args.language ?? 'id',
     args.body, args.status ?? 'draft', args.notes ?? null, args.createdBy],
  );
  const id = rows[0]!.id;
...
export async function updateMessageTemplate(
  ctx: Ctx,
  args: {
    templateId: string; name: string; channel?: TemplateChannel; category?: TemplateCategory; language?: string;
    body: string; status?: TemplateStatus; notes?: string | null; actorId: string;
  },
): Promise<boolean> {
  const rows = await ctx.tx.query<{ id: string }>(
    `update message_templates set
        name = $3, channel = $4, category = $5, language = $6, body = $7, status = $8, notes = $9, updated_at = now()
      where tenant_id = $1 and id = $2
      returning id`,
    [ctx.tenantId, args.templateId, args.name, args.channel ?? 'whatsapp', args.category ?? 'utility',
     args.language ?? 'id', args.body, args.status ?? 'draft', args.notes ?? null],
  );
