...
export default async function ChatIgThreadPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // The conversation this link pointed to is gone — most likely the
    // Instagram account was disconnected, cascading its conversations with
    // it. The list view is still there, so send the agent back to it rather
    // than a dead end.
    if (err instanceof ApiError && err.status === 404) redirect('/chat-ig');
    throw err;
  }
      ...
      <div className="thread" style={{ flex: 1, minWidth: 0 }}>
        <div className="thread-head">
          <div style={{ minWidth: 0 }}>
            <h2 style={{ fontSize: 15 }}>{contact.displayName ?? '—'}</h2>
          </div>
        ...
        {/* Instagram Messaging has its own 24-hour standard-messaging window,
            same idea as WhatsApp's — not yet enforced server-side (no
            template/tag system built for it), so the composer is left open
            rather than showing a restriction it can't actually apply. */}
        <Composer conversationId={conversation.id} windowOpen customerName={contact.displayName ?? '—'}
