...
export async function connectFacebookBridge(_prev: FbBridgeResult | null, form: FormData): Promise<FbBridgeResult> {
  ...
  // Sent as undefined rather than '' so it drops out of the JSON body entirely
  // instead of tripping the API's digit check on an empty field — but checked
  // here first, so a mistyped id fails with a message the operator can act on
  // right away rather than surfacing later as a Business Suite URL that 404s.
