 ...
 *
 * A contact that came from the brand list is BD's: someone is being qualified
 * towards a meeting, and Autopilot would answer them out of a product
 * catalogue they never asked about. Everyone else is Autopilot's.
 */
export async function isBdConversation(
  tx: Sql, tenantId: string, conversationId: string,
): Promise<boolean> {
  const rows = await tx.query<{ one: number }>(
    `select 1 as one
       from conversations c
       join brands b on b.contact_id = c.contact_id and b.tenant_id = c.tenant_id
...
async function applyActions(
  ...
  await withTenant(deps.db, args.tenantId, async (tx) => {
    ...
    for (const send of sends) {
      await queueOutboundMessage(ctx, {
